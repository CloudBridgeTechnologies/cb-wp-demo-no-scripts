...
if( ! function_exists( 'blossom_spa_single_post_schema' ) ) :
/**
 * Single Post Schema
 *
 * @return string
 */
function blossom_spa_single_post_schema() {
    if ( is_singular( 'post' ) ) {
        global $post;
        $custom_logo_id = get_theme_mod( 'custom_logo' );

        $site_logo   = wp_get_attachment_image_src( $custom_logo_id , 'blossom-spa-schema' );
        $images      = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
        $excerpt     = blossom_spa_escape_text_tags( $post->post_excerpt );
        $content     = $excerpt === "" ? mb_substr( blossom_spa_escape_text_tags( $post->post_content ), 0, 110 ) : $excerpt;
        ...
        $args = array(
            ...
            "author"        => array(
                "@type"     => "Person",
                "name"      => blossom_spa_escape_text_tags( get_the_author_meta( 'display_name', $post->post_author ) )
            ),
...
add_action( 'wp_head', 'blossom_spa_single_post_schema' );
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p class="dismiss-link"><strong><a href="?blossom-spa-update-notice=1"><?php esc_html_e( 'Dismiss','blossom-spa' ); ?></a></strong></p>
                </div>
            </div>
        </div>
    <?php }
...
function blossom_spa_ignore_admin_notice() {
    ...
    if ( isset( $_GET['blossom-spa-update-notice'] ) && $_GET['blossom-spa-update-notice'] = '1' ) {
